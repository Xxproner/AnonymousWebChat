...
using ConnectionInfo = Server::ConnectionInfo;
...
int CharactersFilter(const char* c_str, size_t size);
...
void WorkingServerLoop(const Server& registration_server);
...
int main()
{
	...
	Server* ptr_to_server = nullptr;
	Server** ptr_to_ptr_to_server = &ptr_to_server;

#warning "Be carefull with strict aliasing (reinterpret_cast)!"
	// Server registration_server(static_cast<MHD_FLAG>(MHD_USE_DEBUG), 
	// 						 WEBSERVERPORT, &FilterIP, nullptr,
	// 						 &ReplyToConnection, reinterpret_cast<void*>(ptr_to_ptr_to_server),
	...
	ptr_to_server = &registration_server;
...
{
	uint16_t HTTP_STATUS_CODE = session->STATUS_CODE;
	enum MHD_Result ret;
	struct MHD_Response *response;
		...
		strcmphead(page, "/static/") == 0 || page.compare("/") == 0)
	{
		...
		{ 
			if (errno == ENOENT) // no such file or directory
			{
				...
				response = MHD_create_response_from_buffer(strlen(Server::NOT_FOUND_ERROR),
					(void*) Server::NOT_FOUND_ERROR, MHD_RESPMEM_PERSISTENT);
